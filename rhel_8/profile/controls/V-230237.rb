...
control 'V-230237' do
  title "The RHEL 8 pam_unix.so module must use a FIPS 140-2 approved
cryptographic hashing algorithm for system authentication."
...
general-purpose computing system.
  ...
  desc  'check', "
    ...
    Check that pam_unix.so auth is configured to use sha512 in both
/etc/pam.d/password-auth and /etc/pam.d/system-auth with the following command:
    ...
    If \"sha512\" is not an option in both outputs, or is commented out, this
is a finding.
  "
  desc  'fix', "
...
algorithm for system authentication.
    ...
    Edit/modify the following line in the file \"/etc/pam.d/password-auth\" and
\"/etc/pam.d/system-auth\" files to include the sha512 option for pam_unix.so:
