...
control 'V-230251' do
  title "The RHEL 8 SSH daemon must be configured to use only Message
...
can be viewed in the /etc/crypto-policies/back-ends/openssh.config file.
    ...
    By specifying a hash algorithm list with the order of hashes being in a
“strongest to weakest” orientation, the system will automatically attempt to
use the strongest hash for securing SSH connections.
...
finding.
    ...
    Check that the MACs in the back-end configurations are FIPS 140-2-approved
algorithms with the following command:

    $ sudo grep -i macs /etc/crypto-policies/back-ends/openssh.config
/etc/crypto-policies/back-ends/opensshserver.config

    /etc/crypto-policies/back-ends/openssh.config:MACs
...
-oMACS=hmac-sha2-512,hmac-sha2-256'
    ...
    If the MAC entries in the \"openssh.config\" and \"opensshserver.config\"
files have any hashes other than \"hmac-sha2-512\" and \"hmac-sha2-256\", the
order differs from the example above, if they are missing, or commented out,
this is a finding.
  "
  desc  'fix', "
    Configure the RHEL 8 SSH daemon to use only MACs employing FIPS
140-2-approved algorithms with the following commands:

    $ sudo fips-mode-setup --enable

    Next, update the \"/etc/crypto-policies/back-ends/openssh.config\"
    and \"/etc/crypto-policies/back-ends/opensshserver.config\" files to
include these MACs employing FIPS 140-2-approved algorithms:
